...
def main():
    ...
    parser.add_argument('public_key', type=argparse.FileType('rb'), help="Plik zawierający klucz publiczny")
    parser.add_argument('file', type=argparse.FileType('rb'), help="Plik do zaszyfrowania")
    parser.add_argument('-o', dest='filename', type=str, help="Nowa nazwa pliku", required=False)
    parser.add_argument('-t', dest='processes', type=int_range(1, mp.cpu_count()), help="Maksymalna liczba procesów", default = 1, required=False)
    parser.add_argument('-i', dest='info', help="Podsumowanie czasu szyfrowania", action='store_true', required=False)
